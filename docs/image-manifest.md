...
値は改行と制御文字を空白に潰し、512 文字で切ったものが入る。正規化して空になる値と、
prefix なしのラベルは無視する。出力はキーの昇順で安定する。ラベルが 1 つもない場合
`extra` キー自体を出力しない。

改行を潰すのはブロックの構造を守るため。値が複数行になると YAML block scalar として
描画され、その中に `--- END image-updater metadata ---` を書けてしまう。`findManifestBlock`
は最初の END マーカーで打ち切るので、残りが孤児コメントとして居座り、更新ごとにファイルが
伸び続ける。正規化で改行を落としたうえで、描画時にもマーカー行の混入を検査している。
